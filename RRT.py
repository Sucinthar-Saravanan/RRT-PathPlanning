...
#The class Button is defined here which is used as NEXT button
class Button:
...
#Function to check if the point is lies inside the map
...
level=1
ClickText()
DesText("Draw the Obstacles required")
while running:
        ...
        if m[0]==1:
            if point_inside_rec(B1.x,B1.y, B1.width, B1.height,x,y):
                    ...
                    if level==1 and Start==[]:
                        level+=1
                        B1.colour=RED
                        DesText("Choose the starting point")
                    elif level==2 and Start:
                        level+=1
                        B1.colour=GREEN
                        DesText("Choose the destination point")
                    elif level==3 and End!=set():
                        level+=1
                        B1.colour=BLUE
                        DesText("Using RRT algorithm the path is exporled")
                    B1.create(screen)
                    ClickText()
                    continue
            elif level==1:
...
while(Trace and running):
    ...
    while(Trace!=Start):
        x,y = parent[Trace]
        pygame.draw.line(screen, YELLOW, (x,y), Trace, 2)
        Trace=(x,y)
        #print("Trace",x,y)
    DesText("The yellow colour path is the  most optimised path")
    pygame.display.update()
